...
async function buildAll() {
  ...
  // Build 1: Dev server (index.ts) + pino workers — uses outdir (required by pino plugin)
  await esbuild({
  ...
  // Build 2: Serverless entry (serverless.ts) — separate build, no pino plugin,
  // uses outfile so it lands as serverless.mjs alongside the pino worker files.
  await esbuild({
